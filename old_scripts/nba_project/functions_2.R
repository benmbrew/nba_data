# this function combines fantasy and team data. 
# loop through players and create features 
combine_fantasy_team_data <- function(temp_dat){
  player_list <- list()
  player_names <- unique(temp_dat$f_player)
  #loop through each player and create past features
  for(i in 1:length(player_names)){
    player_name <- player_names[i]
    #subset data to get individual players
    sub_player <- temp_dat[temp_dat$f_player == player_name,]
    player_team <- unique(sub_player$f_team)
    # get player team data
    sub_team <- dat_team[dat_team$t_teams %in% player_team,]
    temp_1 <- inner_join(sub_player, sub_team, by = c('f_dataset' = 't_dataset',
                                                      'f_date' = 't_date',
                                                      'f_team' = 't_teams'))
    
    # join with team data by combining players oppening with team
    temp_2 <- inner_join(temp_1, dat_team, by = c('f_dataset' = 't_dataset',
                                                  'f_date' = 't_date',
                                                  'f_opp_team' = 't_teams'))
    
    
    
    # replace .x with player_team and .y with opp_team
    names(temp_2) <- gsub('.x', '_player_team', names(temp_2), fixed = T)
    names(temp_2) <- gsub('.y', '_opp_team', names(temp_2), fixed = T)
    
    # store in list
    player_list[[i]] <- temp_2
    print(i)
    
  }
  final_dat <- do.call('rbind', player_list)
  return(final_dat)
