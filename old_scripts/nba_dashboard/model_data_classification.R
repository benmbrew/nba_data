...
# subset mod dat by dataset
mod_dat <- mod_dat[grepl('2017|2018', mod_dat$dataset),]
...
get_folds <- function(temp_dat){
  ...
  for (i in 1:length(unique_dataset)){
    ...
    if(this_dataset == '2014_2015_season'){
      ...
      sub_dat$fold <- ifelse(sub_dat$month == 10, 8, 
                             ifelse(sub_dat$month == 11, 9, 
                                    ifelse(sub_dat$month == 12, 10, 11)))
    }
...
# # extract test set 
test_dat <- mod_dat[mod_dat$fold == 11,]
...
train_window = c(1:9)
test_window =c(10:11)
response_variable = 'real_total_fac'
include_ou = TRUE
...
pred_team_regression <- function(model_matrix,
  ...
  if(model_type == 'random_forest') {
    ...
    temp <- varImp(model)[[1]]
    importance <- cbind(variable = rownames(temp), positive = temp$positive)
    importance <- as.data.frame(importance)
    importance$positive <- round(as.numeric(as.character(importance$positive)), 2)

    importance <- importance %>% arrange(-positive)
...
temp$pred_class <- as.factor(ifelse(temp$predicted > 0.5, 'positive', 'negative'))
temp$pred_class <- factor(temp$pred_class, levels = c('positive', 'negative'))
temp$real <- as.factor(ifelse(temp$real_y == 1, 'positive', 'negative'))
temp$real <- factor(temp$real, levels = c('positive', 'negative'))
caret::confusionMatrix(table(temp$pred_class, temp$real))


temp_dat$real_y <- factor(temp_dat$real_y, levels = c('positive', 'negative'))
temp_dat$predicted <- factor(temp_dat$predicted, levels = c('positive', 'negative'))
